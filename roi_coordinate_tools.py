...
def map_coordinate(original_coords_file, reduced_roi_file, save_directory=".", name_of_file=None):
    ...
    try:
        ...
        mapped_results = []
        unmapped_rois = []  # Track ROIs that couldn't be mapped
...
def load_and_clean_coordinates(csv_file_path, output_file_name=None, save_directory="."):
    ...
    output_file_name : str, optional
        Name for the output files (without extension). If None, uses original filename + '_cleaned'
    save_directory : str, optional
        Directory where cleaned files will be saved. Default is current directory.
    ...
    try:
        ...
        # Identify rows with missing coordinate values
        ...
        # Find rows with NaN values in coordinate columns
...
def load_matrix_replace_nan(file_path, replacement_value=0):
    ...
    try:
        if file_ext == '.mat':
            ...
            # Find the matrix variable (skip metadata keys)
...
def test_functions():
    ...
    print("  - load_and_clean_coordinates(csv_file, output_name, save_dir)")
    print("  - load_matrix_replace_nan(file_path, replacement_value)")
