...
> Markdown sibling of [`legend key and 3 view display test.ipynb`](legend%20key%20and%203%20view%20display%20test.ipynb).
...
(see [PVALUE_PLOTTING_TUTORIAL.md](PVALUE_PLOTTING_TUTORIAL.md) for
...
![6-panel 2×3 grid of canonical brain views](../docs/images/legend_tutorial/12_mv_grid.png)
...
`compose_image_grid` (Python) or `hlplot montage` (CLI). It is the same Pillow
back-end as the multi-view stitcher (auto-crop, uniform pad, labels), but it takes
a list of **pre-rendered PNGs** instead of one figure, and adds per-column headers
and per-row labels.
...
compose_image_grid(
    ...
    output_path=str(OUTPUT_DIR / 'species_grid.png'),
    grid=(2, 3),
...
> **`--multi-view-grid` vs `montage`:** `--multi-view-grid` lays out camera views
> of **one** scene; `hlplot montage` composes **independent** images (different
> meshes, conditions, subjects — anything). The worked human/rat/macaque example
> lives in [FIGURE_CREATION.md](FIGURE_CREATION.md).
...
![5-panel canonical-views strip from the standalone helper](../docs/images/legend_tutorial/11_helper_standalone.png)
...
*Companion notebook: [`legend key and 3 view display test.ipynb`](legend%20key%20and%203%20view%20display%20test.ipynb) — runs every snippet above end-to-end and is the source of truth for the rendered images in `docs/images/legend_tutorial/`.*
